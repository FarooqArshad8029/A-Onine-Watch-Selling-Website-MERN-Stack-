...
export default function MessegingPage() {
  ...
  useEffect(() => {
    async function fetchChatMessages() {
      try {
        const response = await axios.get(`/get-user-chat/${auth?.user?._id}`);
  ...
  const send = () => {
    if (msg.trim() !== "") {
      ...
      socket.emit("OnSendingUserMessage", {
        message: msg,
        socketId: id,
  ...
  useEffect(() => {
    ...
    if (auth.token) {
      ...
      newSocket.on("recieveSendedUserMessage", (data) => {
        console.log("sendMessage:", data);
        setRecieveMsg((prevMessages) => [...prevMessages, data]);
      });
      // Listen for messages from the server
      newSocket.on("recieveSendedAdminmessage", (data) => {
        setRecieveMsg((prevMessages) => [...prevMessages, data]);
      });
